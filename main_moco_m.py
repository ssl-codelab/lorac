...
def train(train_loader, model, optimizer, epoch, lr_schedule):
    ...
    for it, inputs in enumerate(train_loader):
        ...
        for i, crop_id in enumerate(args.crops_for_assign):
            ...
            # infonce loss with multi-crop augmentation
            _q = q.reshape(total_crop, bs, -1)[np.delete(np.arange(total_crop), 1), ...].reshape(-1, q.shape[1])
            _k = k_list[1 - crop_id].repeat(total_crop - 1, 1)
            ...
            loss += F.cross_entropy(logits, labels) / len(args.crops_for_assign)
