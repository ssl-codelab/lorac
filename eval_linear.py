...
import models
from src.logger import SummaryWriter
...
parser.add_argument("--model_prefix", default='module', type=str, help="model prefix")
...
def main():
    ...
    # build model
    model = models.__dict__[args.arch](num_classes=0, eval_mode=True)
    ...
    if os.path.isfile(args.pretrained):
        ...
        # remove prefix
        state_dict = {k.replace(f"{args.model_prefix}.", ""): v for k, v in state_dict.items() if k.startswith(args.model_prefix)}
...
def train(model, reglog, optimizer, loader, epoch):
    ...
    # training statistics
    top1 = PaceAverageMeter()
    top5 = PaceAverageMeter()
    ...
    for iter_epoch, (inp, target) in enumerate(loader):
        ...
        left_time.update(batch_time.val * ((args.epochs - epoch) * len(loader) - iter_epoch) / 3600.0)
        end = time.perf_counter()

        # verbose
        if args.rank == 0 and (iter_epoch + 1) % 100 == 0:
            logger.info(
                "Epoch[{0}] - Iter: [{1}/{2}]\t"
            ...
            writer.log(iter=iter, tag='train/loss', val=losses.avg)
            writer.log(iter=iter, tag='train/top1', val=top1.avg)
            writer.log(iter=iter, tag='train/top5', val=top5.avg)
            writer.log(iter=iter, tag='train/lr', val=optimizer.param_groups[0]["lr"])
...
def validate_network(val_loader, model, linear_classifier, epoch):
    ...
    if args.rank == 0:
        ...
        writer.log(iter=epoch, tag='val/loss', val=losses.avg)
        writer.log(iter=epoch, tag='val/top1', val=top1.avg)
        writer.log(iter=epoch, tag='val/top5', val=top5.avg)
